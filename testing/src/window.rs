...
pub struct WebViewController(ICoreWebView2Controller);
...
#[derive(Clone)]
pub struct WebView {
  pub controller: Arc<WebViewController>,
  pub webview: Arc<ICoreWebView2>,
  pub tx: WebViewSender,
  pub rx: Arc<WebViewReceiver>,
  pub thread_id: u32,
  pub bindings: Arc<Mutex<BindingsMap>>,
  pub frame: Option<FrameWindow>,
  pub parent: Arc<HWND>,
...
impl WebView {
        ...
        &WebMessageReceivedEventHandler::create(Box::new(move |_webview, args| {
          if let Some(args) = args {
            ...
            if args.WebMessageAsJson(&mut message).is_ok() {
              ...
              if let Ok(value) = serde_json::from_str::<InvokeMessage>(&message.to_string()) {
                if let Ok(mut bindings) = bindings.try_lock() {
                  if let Some(f) = bindings.get_mut(&value.method) {
                    match (*f)(value.params) {
                      ...
                      Err(err) => bound.resolve(value.id, 1, Value::String(format!("{err:#?}"))),
                    }
                    .unwrap();
                  }
                }
  ...
  pub fn run(self) -> Result<(Self, MSG, HWND)> {
